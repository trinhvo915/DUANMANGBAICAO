	...
	public void layoutContainer(Container parent) {
		...
		if(check)
		{
			...
			for(int i = 0 ;i < parent.getComponentCount();i++)
			{
				...
				c.setLocation(x + ((gap + c.getWidth()) * ( c.getCardData().getNuocCo() -1)) ,y + (v * row[c.getCardData().getNuocCo() - 1]));
		
				row[c.getCardData().getNuocCo() - 1] ++ ;
		...
		else
		{
			for(int i = 0 ;i < parent.getComponentCount();i++)
			{
				...
				if(c.getTableIndex() == 0)
				{
					c.setLocation(x,y + c.getHeight() + gap);
				}
				else if(c.getTableIndex() == 2)
				{
					c.setLocation(x,y - (c.getHeight() + gap));
				}
				else if(c.getTableIndex() == 1)
				{
					c.setLocation(x  + c.getWidth() + gap ,y);
				}
				else if(c.getTableIndex() == 3)
				{
					c.setLocation(x  -( c.getWidth() + gap) ,y);
				}
			}
